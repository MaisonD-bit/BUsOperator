...
class ScheduleController extends Controller
{
    ...
    /**
     * Get only active schedules with route details for commuter app
     * Returns schedules where status = 'active' (driver has started the trip)
     */
    public function getActiveSchedules(): JsonResponse
    {
        try {
            // Get schedules with status 'active' and load route details including geometry
            $activeSchedules = Schedule::with(['route', 'bus', 'driver'])
                ->where('status', 'active')
                ->whereDate('date', Carbon::today()) // Only today's schedules
                ->get();

            // Format the response for commuter app
            $formattedSchedules = $activeSchedules->map(function ($schedule) {
                return [
                    ...
                    'geometry' => $schedule->route->geometry ? json_decode($schedule->route->geometry) : null,
                    'distance_km' => $schedule->route->distance_km,
                    'regular_price' => $schedule->route->regular_price,
                    'aircon_price' => $schedule->route->aircon_price,
                    'driver_name' => $schedule->driver->name ?? 'Unknown',
                    'bus_plate_number' => $schedule->bus->plate_number ?? 'Unknown',
            ...
            return response()->json([
                'success' => true,
                'active_schedules' => $formattedSchedules,
                'count' => $formattedSchedules->count()
            ]);


        } catch (\Exception $e) {
            Log::error("Error fetching active schedules: " . $e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'Error fetching active schedules: ' . $e->getMessage()
            ], 500);
        }
    }
}
