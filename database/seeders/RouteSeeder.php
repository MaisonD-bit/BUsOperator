...
class RouteSeeder extends Seeder
{
    ...
    private function createRoute($name, $code, $start, $end, $description, $regularPrice, $airconPrice, $distance, $duration, $busCompany)
    {
        ...
        return Route::create([
            ...
            'status' => 'active',
            'geometry' => $this->generateRouteGeometry($start, $end, $startStop, $endStop),
        ]);
    }

    private function generateRouteGeometry($start, $end, $startStop, $endStop)
    {
        ...
        // Generate realistic multi-point routes based on typical Cebu road paths
        $routeKey = $start . '_to_' . $end;
        $coordinates = [];

        switch ($routeKey) {
            case 'Cebu North Bus Terminal_to_Borbon':
                // Cebu -> Sogod -> Borbon route with intermediate points
                $coordinates = [
                    [123.9172, 10.3156], // Cebu North Bus Terminal
                    [123.9500, 10.4000], // Intermediate point 1
                    [123.9800, 10.5000], // Intermediate point 2
                    [124.0014, 10.7292], // Sogod
                    [124.0271, 10.8347], // Borbon
                ];
                break;

            case 'Cebu North Bus Terminal_to_Tabogon':
                // Cebu -> Tuburan -> Tabogon route
                $coordinates = [
                    [123.9172, 10.3156], // Cebu North Bus Terminal
                    [123.8500, 10.4000], // Intermediate point 1
                    [123.8291, 10.7303], // Tuburan
                    [123.9000, 10.8500], // Intermediate point 2
                    [123.9978, 10.9569], // Tabogon
                ];
                break;

            case 'Cebu North Bus Terminal_to_Tabuelan':
                // Cebu -> Maravilla -> Tabuelan route
                $coordinates = [
                    [123.9172, 10.3156], // Cebu North Bus Terminal
                    [123.8800, 10.4500], // Intermediate point 1
                    [123.9296, 10.7893], // Maravilla
                    [123.9478, 10.8687], // Tabuelan
                ];
                break;

            case 'Cebu North Bus Terminal_to_Daan Bantayan Maya':
                // Cebu -> Kawit -> Bagay -> Maya route
                $coordinates = [
                    [123.9172, 10.3156], // Cebu North Bus Terminal
                    [123.9421, 10.9971], // Kawit
                    [123.9700, 11.0500], // Bagay
                    [124.0122, 11.3181], // Maya
                ];
                break;

            case 'Cebu North Bus Terminal_to_Hagnaya':
                // Cebu -> Hagnaya route
                $coordinates = [
                    [123.9172, 10.3156], // Cebu North Bus Terminal
                    [123.8800, 10.4500], // Intermediate point 1
                    [123.8500, 10.6000], // Intermediate point 2
                    [123.8244, 11.1223], // Hagnaya
                ];
                break;

            case 'Cebu North Bus Terminal_to_Tacloban':
                // Cebu -> Sogod -> Tacloban route (simplified)
                $coordinates = [
                    [123.9172, 10.3156], // Cebu North Bus Terminal
                    [124.0014, 10.7292], // Sogod
                    [124.2000, 10.9000], // Intermediate point 1
                    [124.5000, 11.1000], // Intermediate point 2
                    [125.0037, 11.2429], // Tacloban
                ];
                break;

            default:
                // Default: just start and end points
                $coordinates = [
