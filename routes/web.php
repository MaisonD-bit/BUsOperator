...
// Simple payment result pages (public)
Route::get('/payment/success', function (\Illuminate\Http\Request $request) {
    $ref = $request->query('requestReferenceNumber') ?: $request->query('ref');
    return response()->make("<html><body><h1>Payment Successful</h1><p>Reference: {$ref}</p><p>You may close this window and return to the app.</p></body></html>");
});

Route::get('/payment/failure', function (\Illuminate\Http\Request $request) {
    $ref = $request->query('requestReferenceNumber') ?: $request->query('ref');
    return response()->make("<html><body><h1>Payment Failed</h1><p>Reference: {$ref}</p><p>Please try again or contact support.</p></body></html>");
});
